...
def Where(tuning,train,test):
    global The
    assert(len(tuning) != 0),"parameters are empty"
    The.tree.infoPrune = round(float(tuning[0]), 2)
    The.tree.min = int(tuning[1])
    The.option.threshold = round(float(tuning[2]), 2)
    The.where.wriggle = round(float(tuning[3]), 2)
    The.where.depthMax = int(tuning[4])
    The.where.depthMin = int(tuning[5])
    The.option.minSize = round(float(tuning[6]), 2)
    if (tuning[7] - 0.5) > 0: 
      The.tree.prune = True
    else:
      The.tree.prune = False
    ...
    #print "Tuning: ", The.tree.infoPrune, The.tree.min, The.option.threshold, The.where.wriggle, The.where.depthMax ,The.where.depthMin, The.option.minSize,The.tree.prune, The.where.prune
